...
static GLintPoint juice[12];	//	Global Static Array used to store sphere coordinates
...
void myDisplay(void)
{
	...
	glLoadIdentity();
	gluLookAt(0, 0, 0, 0, 5, 0, 0, 0, 10);	//	eye (3,3,0); look at reference (2.5, 2.5, 10); up (2.5, 10, 2.5) 

	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);		// clear the screen
	GLintPoint spheres[12];
	for (int i = 0; i < 12; i++)	//draw initial spheres
	{
		spheres[i].x = (rand() % 10) + 10;		//	randomizing initial values for x,y,z of starting location
		spheres[i].y = (rand() % 10) + 10;
...
void displaySolid(void)
{
	glutKeyboardFunc(myKeyboard);	// keyboard function to quit
		...
		{			
			if (juice[i].z > 5)		//	if still behind near plane
			{
				juice[i].z -= 0.01;
			}
			else {
				juice[i].z = 20;		// else reset current sphere to far plane, & randomize x and y
				juice[i].x = (rand() % 10) + 10;
				juice[i].y = (rand() % 10) + 10;
			}

			//if x is larger than camera, -=

			if (juice[i].y > 5) {
				juice[i].y = (juice[i].y - 0.1);
			}
			else
				juice[i].y = (juice[i].y + 0.1);
			
			if (juice[i].x > 5) {
				juice[i].x = (juice[i].x - 0.1);
			}
			else
				juice[i].x = (juice[i].x + 0.1);
			...
			glMaterialfv(GL_FRONT, GL_AMBIENT, mat_ambient);
			sphere(juice[i].x, juice[i].y, juice[i].z, 0.3, 15, 15);
		}
